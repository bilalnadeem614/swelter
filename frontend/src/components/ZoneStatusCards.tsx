      ...
      {zones.map((zone, i) => {
        ...
        return (
          <button key={zone.id} onClick={() => onSelectZone(zone.id)} className="text-left">
            <Card className={`h-full border-l-4 ${ACTION_BORDER[action]} transition-colors hover:bg-muted/40`}>
              <div className="flex items-start justify-between gap-2 px-4">
                <span className="text-xs text-muted-foreground">ZONE {String(i + 1).padStart(2, "0")}</span>
